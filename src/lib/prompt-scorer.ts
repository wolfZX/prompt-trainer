import { PromptAnalysis, PromptFeedback, PromptCategory, PromptQuality, TOKEN_LIMITS } from '@/types';

export class PromptScorer {
  private static readonly KEYWORDS = {
    technical: ['code', 'function', 'algorithm', 'debug', 'implement', 'optimize', 'refactor', 'api', 'database', 'framework'],
    creative: ['story', 'creative', 'imagine', 'design', 'artistic', 'innovative', 'brainstorm', 'concept', 'vision'],
    analytical: ['analyze', 'compare', 'evaluate', 'research', 'study', 'examine', 'investigate', 'assess'],
    conversational: ['explain', 'discuss', 'tell', 'chat', 'talk', 'conversation', 'dialogue'],
    instructional: ['how', 'step', 'guide', 'tutorial', 'teach', 'learn', 'instructions', 'process'],
    coding: ['javascript', 'python', 'react', 'html', 'css', 'programming', 'syntax', 'variable', 'class', 'method']
  };

  private static readonly QUALITY_INDICATORS = {
    positive: [
      'specific', 'detailed', 'clear', 'precise', 'context', 'example', 'format',
      'tone', 'audience', 'goal', 'constraint', 'requirement', 'criteria'
    ],
    negative: [
      'anything', 'something', 'stuff', 'things', 'whatever', 'maybe', 'possibly',
      'kinda', 'sorta', 'idk', 'dunno', 'just', 'simply'
    ]
  };
  ...
  static analyzePrompt(prompt: string): PromptAnalysis {
    ...
    return {
      score: overallScore,
      tokenCount,
      isNearLimit: tokenCount > TOKEN_LIMITS.GPT3_5 * TOKEN_LIMITS.WARNING_THRESHOLD,
      isAtLimit: tokenCount > TOKEN_LIMITS.GPT3_5 * TOKEN_LIMITS.ERROR_THRESHOLD,
      feedback,
      suggestions,
      category,
      quality
  ...
  private static createEmptyAnalysis(): PromptAnalysis {
    return {
      ...
      isNearLimit: false,
      isAtLimit: false,
      feedback: { clarity: 0, specificity: 0, context: 0, structure: 0, length: 0 },
  ...
  private static assessClarity(prompt: string): number {
    ...
    const fillerWords = (prompt.match(/\b(um|uh|like|you know|basically|actually)\b/gi) || []).length;
    score -= fillerWords * 5;
  ...
  private static assessSpecificity(prompt: string): number {
    ...
    if (/\b(json|csv|html|markdown|bullet points|numbered list)\b/i.test(prompt)) score += 10;

    // Constraints and requirements
    if (/\b(must|should|need|require|limit|maximum|minimum)\b/i.test(prompt)) score += 10;

    // Vague words reduce specificity
    const vagueWords = (prompt.match(/\b(some|many|few|several|various|different|general)\b/gi) || []).length;
    score -= vagueWords * 3;
  ...
  private static assessContext(prompt: string): number {
    ...
    // Context indicators
    if (/\b(for|to|in order to|because|since|given that|assuming)\b/i.test(prompt)) score += 15;
    
    // Audience mentioned
    if (/\b(audience|reader|user|student|beginner|expert|professional)\b/i.test(prompt)) score += 10;
    
    // Purpose or goal mentioned
    if (/\b(goal|purpose|objective|aim|target|result|outcome)\b/i.test(prompt)) score += 10;
    
    // Background information
    if (/\b(background|context|situation|scenario|case|project)\b/i.test(prompt)) score += 8;
  ...
  private static assessStructure(prompt: string, sentences: string[]): number {
    ...
    // Logical flow words
    if (/\b(first|second|then|next|finally|however|therefore|additionally)\b/i.test(prompt)) score += 8;
  ...
  private static categorizePrompt(prompt: string): PromptCategory {
    ...
    return Object.entries(scores).find(([, score]) => score === maxScore)?.[0] as PromptCategory || PromptCategory.CONVERSATIONAL;
  }

  private static generateSuggestions(prompt: string, feedback: PromptFeedback): string[] {
    ...
    if (feedback.length < 50) {
      if (prompt.split(/\s+/).length < 5) {
        ...
        suggestions.push("Consider breaking this into shorter, focused requests");
      }
    }
